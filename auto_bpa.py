...
status = 0 初始化准备
status = 1 用户配置读取成功
status = 2 AES加密模块读取成功
status = 3 AES加密模块读取失败
status = 4 用户配置读取失败
'''
...
class Userdata:
    ...
    def read_user_data(self):
        try:
            with open('./user_data.txt', 'r', encoding = 'utf-8') as f:  # r 只读打开
                self.user_data = f.readlines()
                self.status = 1
                print(time.strftime("%H:%M:%S", time.localtime()) + "用户数据读取成功，共读取到%d条数据！" % len(self.user_data))
...
class MainControl:
    def __init__(self, all_user_data):
        ...
        for user in all_user_data:
            user = list(user)
            user[2] = self.get_aes_passwd(aesjs, str(user[2]))
            self.log = "------------------报平安自动化日志------------------\n1#您的序列号为：" + str(user[0]) + '\n2#您的密码AES加密后为：' + str(user[2])  # 日志文件
            try:
                self.login(user)
                self.commit(user)
                print(time.strftime("%H:%M:%S", time.localtime()) + "用户%s学号%s执行完成！" % (user[3], user[1]))
                self.send_mail(user)
            except:
                print(time.strftime("%H:%M:%S", time.localtime()) + "用户%s学号%s执行失败！" % (user[3], user[1]))

    def get_aes_passwd(self, js, t):  # AES 密码加密
        aes = execjs.compile(js)  # 加载JS文件
        return aes.call('Encrypt', str(t))  # 调用js方法  第一个参数是JS的方法名，后面的data和key是js方法的参数

    def get_chinese(self, t):
        a = t
        t = ''
        for i in a:
            if u'\u4e00' <= i <= u'\u9fff':
    ...
    def login(self, user):
        ...
        # 直接进行获取
        self.sessionid_cookie = re.search('JSESSIONID=\w+', requests.history[1].headers['Set-Cookie']).group()
        print("会话Cookie获取成功：" + self.sessionid_cookie)
        self.log = self.log + "\n4#会话Cookie获取成功：" + str(self.sessionid_cookie)
        # print(requests.history[1].headers['Set-Cookie'])
        self.sessionid = re.search('(?<=get sessionID\(\) {return \')\d+(?=\'})', requests.text).group()
        print("Sessionid获取成功：" + self.sessionid)
        self.log = self.log + "\n5#Sessionid获取成功：" + str(self.sessionid)
        # self.cookie = requests.history[1].headers['Location']
        # print("提交Cookie获取成功(网页内地址，存在问题)  " '%s' % self.cookie)
        ...
        if title_check == "<title>报平安</title>":
            self.sessionid_cookie = re.search('JSESSIONID=\w+', requests.history[1].headers['Set-Cookie']).group()
            print("会话Cookie获取成功  " '%s' % self.sessionid_cookie)
            #print(requests.history[1].headers['Set-Cookie'])
            self.sessionid = re.search('(?<=get sessionID\(\) {return \')\d+(?=\'})', requests.text).group()
            print("Sessionid获取成功  "'%s'%self.sessionid)
            self.cookie = requests.history[1].headers['Location']
            print("提交Cookie获取成功(网页内地址，存在问题)  " '%s'%self.cookie)
    ...
    def commit(self, user):
        print("开始提交数据！")
        # commit_url = 'https://bdmobile.zjtongji.edu.cn:8081/ReportServer;jsessionid='+ re.search(r'(?<=JSESSIONID=).+',self.sessionid_cookie).group()
        ...
        self.log = self.log + "\n↓POST请求响应检查点↓ \n如果下面行为空说明提交失败！有 'callback' 字样表示成功\n"
        print(requests.text)
        self.log = self.log + requests.text
        print("↑↑↑↑↑↑↑↑↑↑表单数据提交完毕检查点↑↑↑↑↑↑↑")
        ...
        commit2_headers = {
            ...
            'Sec-Fetch-User':'?1',
            'Sec-Fetch-Dest':'document',
            'Referer':'https://bdmobile.zjtongji.edu.cn:8081/ReportServer;jsessionid=' + re.search(r'(?<=JSESSIONID=).+', self.sessionid_cookie).group() + '?formlet=xxkj/mobile/bpa/bpa.frm&op=h5',
            'Accept-Encoding':'gzip, deflate, br',
            'Accept-Language':'zh-CN,zh-TW;q=0.9,zh;q=0.8,en-US;q=0.7,en;q=0.6,zh-HK;q=0.5',
            'Cookie':self.sessionid_cookie
        }
        ...
        self.sessionid_2 = re.search('(?<=get sessionID\(\) {return \')\d+(?=\'})', requests.text).group()
        print("Sessionid_2获取成功  "'%s' % self.sessionid_2)
        self.log = self.log + "\n8#Sessionid_2获取成功  "'%s' % self.sessionid_2
        ...
        commit_body = r'__device__=unknown&op=h5_template&sessionID=' + self.sessionid_2 + '&cmd=firstdata&'
        requests = req.post(url = commit_url, headers = commit_headers, data = commit_body, timeout = 4, verify = True)
        print('第4次post页面 :\n' + requests.text)
        self.log = self.log + '\n10#第4次post页面 :\n' + requests.text

        commit_body = r'__device__=unknown&toVanCharts=true&op=page_content&cmd=json&sessionID=' + self.sessionid_2 + '&pn=1&path=&'
        requests = req.post(url = commit_url, headers = commit_headers, data = commit_body, timeout = 4, verify = True)
        print('第5次post页面省略\n')
        # self.log = self.log + '\n第5次post页面 :\n' + requests.text
        self.log = self.log + '\n11#第5次post页面 :\n' + '省略'
...
if __name__ == '__main__':
    ...
    if status == 1:
        pass
        MAIN = MainControl(user_data)
        print('执行完毕！status=' + str(status))
    else:
        print('执行失败！status=' + str(status))
